...
Desarrollar en C subprogramas que a partir de los datos de VIAJES.DAT, den un único recorrido del mismo genere una matriz MP con la cantidad total de pasajeros (línea y turno) y un vector VK con los kilómetros totales por línea.
...
Crear en un subprograma un archivo de texto RESUMEN.TXT con los siguientes datos: Línea (no se repite y solo se deben incluir las líneas que hayan realizado viajes en los turnos). TotalPasajeros, TotalKm.
y solo se deben incluir las líneas que hayan realizado viajes en los turnos.
(Leído en el main) pasajeros entre los turnos.

b)
Hallar mediante una función y luego informar si al menos una línea ha llevado más de X pasajeros entre los tres turnos.
...
entre dichas líneas, cuál fue la que registró la mayor cantidad de pasajeros
